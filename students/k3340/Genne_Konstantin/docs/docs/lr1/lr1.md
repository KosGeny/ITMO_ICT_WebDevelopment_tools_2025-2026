...
| Метод | Путь | Описание | `response_model` | Доступ |
|-------|------|----------|-----------------|--------|
| `POST` | `/register` | Регистрация нового пользователя | `UserOut` | Публичный |
| `POST` | `/login` | Вход в систему, получение JWT | `Token` | Публичный |
| `GET`  | `/me` | Данные текущего пользователя | `UserOut` | Авторизованный |
| `PUT`  | `/change-password` | Смена пароля текущего пользователя | `dict` (`200 OK`) | Авторизованный |
